...
Как Менеджер я хочу
проверить создания нового документа "Обслуживание клиента" от имени менеджера
чтобы убедиться в работе ПО    
...
Структура сценария: Я проверяю корректность создания нового документа "Обслуживание клиента" от имени менеджера
	...
	*И реквизит «Ответственный» автоматически заполнился текущим пользователем
		И у элемента с именем 'Ответственный' я жду значения "Васильевна Менеджер" в течение 20 секунд
	...
	*В поле Договор заполняю Договор
		...
		Если таблица "Список" не содержит строки: Тогда
			...
			# Заполняем даты договора через выражение 1С без использования переменных
			И в поле с именем 'ВКМ_ДатаНачалаДействияДоговора' я ввожу значение выражения "ТекущаяДата()"
			И в поле с именем 'ВКМ_ДатаОкончанияДействияДоговора' я ввожу значение выражения "ТекущаяДата() + 2592000"
	...
	*В поле ДатаПроведенияРабот вводим ДатуПроведенияРабот
		И в поле с именем 'ДатаПроведенияРабот' я ввожу значение выражения "ТекущаяДата() + 86400"
...
| "Тестовый Контрагент 1" | "Договор абонентского обслуживания № 4531 для Тестовый Контрагент 1" | "Машинный Александр Петрович" | "2000"                      | "500"                    |
| "Тестовый Контрагент 2" | "Договор абонентского обслуживания № 7182 для Тестовый Контрагент 2" | "Машинный Александр Петрович" | "1000"                      | "1000"                   |
| "Тестовый Контрагент 3" | "Договор абонентского обслуживания № 9354 для Тестовый Контрагент 3" | "Машинный Александр Петрович" | "3000"                      | "250"                    |