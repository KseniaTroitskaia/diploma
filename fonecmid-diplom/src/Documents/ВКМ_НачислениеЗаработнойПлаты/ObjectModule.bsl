...
Процедура СформироватьДвиженияПоОкладу()
	...
	Запрос.Текст =
		...
		|		ПО ВКМ_НачислениеЗаработнойПлатыСписокСотрудников.Сотрудник = ВКМ_УсловияОплатыСотрудниковСрезПоследних.Сотрудник
		|ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Оклад = 0 Тогда
			ТекстСообщения = СтрШаблон("Внимание! Не задан оклад сотруднику ""%1"".", ВыборкаДетальныеЗаписи.Сотрудник);
...
Процедура РассчитатьОклад()
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ВКМ_ОсновныеНачисленияДанныеГрафика.ДнейПериодДействия, 0) КАК ДнейНорма,
		|	ЕСТЬNULL(ВКМ_ОсновныеНачисленияДанныеГрафика.ДнейФактическийПериодДействия, 0) КАК ДнейФакт
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Показатель = ?(ТипЗнч(Движение.Показатель) = Тип("Число"), Движение.Показатель, 0);
		ДнейНорма = ВыборкаДетальныеЗаписи.ДнейНорма;
		ОтработаноДней = ВыборкаДетальныеЗаписи.ДнейФакт;
		
		Движение.ОтработаноДней = ОтработаноДней;
		
		Если ДнейНорма > 0 Тогда
			Движение.Результат = Показатель * ОтработаноДней / ДнейНорма;
		Иначе
...
Процедура РассчитатьОтпуск()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		// Значения уже числовые благодаря ЕСТЬNULL в запросе
		ДнейФакт = ВыборкаДетальныеЗаписи.ДнейФактическийПериодДействия;
		РезультатБаза = ВыборкаДетальныеЗаписи.РезультатБаза;
		ОтработаноДнейБаза = ВыборкаДетальныеЗаписи.ОтработаноДнейБаза;
		
		Движение.ОтработаноДней = ДнейФакт;
		
		Если ОтработаноДнейБаза > 0 Тогда
			Движение.Показатель = РезультатБаза / ОтработаноДнейБаза;
			Движение.Результат = Движение.Показатель * ДнейФакт;
		Иначе 
...
Процедура РассчитатьНДФЛИВыплаты()
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТ_Доп
		|ИЗ
		|	РегистрРасчета.ВКМ_Удержания.БазаВКМ_ДополнительныеНачисления(&Измерения, &Измерения, &Разрезы, Регистратор = &Ссылка И ВидРасчета = &НДФЛ) КАК ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления
		|ГДЕ
		...
		|	ЕСТЬNULL(ВТ_Доп.РезультатБазаДоп, 0) + ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.РезультатБаза, 0) КАК ПоказательБазы,
		...
		|			Регистратор = &Ссылка И ВидРасчета = &НДФЛ) КАК ВКМ_УдержанияБазаВКМ_ОсновныеНачисления
		|			ПО ВКМ_Удержания.Сотрудник = ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.Сотрудник
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Доп КАК ВТ_Доп
		|			ПО ВКМ_Удержания.Сотрудник = ВТ_Доп.Сотрудник,
	...
	Пока Выборка.Следующий() Цикл
		НачисленоБаза = Выборка.ПоказательБазы;
		НачисленоНДФЛ = НачисленоБаза * Выборка.НДФЛКоэффициент;
		СуммаКВыплате = НачисленоБаза - НачисленоНДФЛ;
		
		// Обходим коллекцию движений удержаний и заполняем существующие ресурсы
		Для Каждого ДвижениеУд Из Движения.ВКМ_Удержания Цикл
			Если ДвижениеУд.Сотрудник = Выборка.Сотрудник И ДвижениеУд.ВидРасчета = Выборка.ВидРасчета Тогда
				ДвижениеУд.Показатель = НачисленоБаза; // Записываем в реквизит Показатель
				ДвижениеУд.Результат = НачисленоНДФЛ; // Записываем в ресурс Результат
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		// Отражаем выплату в регистре накопления
	...
	// Окончательно записываем рассчитанные удержания и выплаты в базу данных
